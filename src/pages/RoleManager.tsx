...
import type { UserRole, RolePermission } from '@/mock/interfaces';
...
const RoleManager = () => {
  const { t } = useLanguage();
  const [roles, setRoles] = useState<Record<string, RolePermission>>({
    Site_Inspector: {
      role: 'Site_Inspector' as UserRole,
  ...
  const handleCreateRole = () => {
    ...
    const roleKey = newRoleName.replace(/\s+/g, '_') as UserRole;
    if (roles[roleKey]) {

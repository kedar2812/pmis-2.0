...
import type { Workflow, WorkflowStep, UserRole } from '@/mock/interfaces';

const userRoles: UserRole[] = ['SPV_Official', 'PMNC_Team', 'EPC_Contractor', 'Consultant_Design', 'Govt_Department', 'NICDC_HQ'];
const actions: WorkflowStep['action'][] = ['Verify', 'Approve', 'Review', 'Sign', 'Submit', 'Reject'];

interface SortableStepProps {
  step: WorkflowStep;
  index: number;
  onDelete: (index: number) => void;
  onUpdate: (index: number, field: keyof WorkflowStep, value: any) => void;
}

const SortableStep = ({ step, index, onDelete, onUpdate }: SortableStepProps) => {
    ...
    <div ref={setNodeRef} style={style} className="flex items-center gap-3 p-4 bg-white border border-slate-200 rounded-xl">
      ...
      <div className="flex-1 grid grid-cols-3 gap-4">
        ...
        <div>
          ...
          <select
            value={step.action}
            onChange={(e) => onUpdate(index, 'action', e.target.value as WorkflowStep['action'])}
            className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"
          >
            {actions.map((action) => (
...
const WorkflowConfig = () => {
  ...
  const [workflowType, setWorkflowType] = useState<Workflow['type']>('Custom');
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <select
                      value={workflowType}
                      onChange={(e) => setWorkflowType(e.target.value as Workflow['type'])}
                      className="w-full px-4 py-2 border border-slate-300 rounded-lg"
                    >

...
import type { Project, Budget, KPI, Risk, Task, CostForecast } from '@/mock/interfaces';
 ...
 */
export const calculateCostVariation = (budget: number, spent: number): number => {
  return budget - spent;
};
 ...
 */
export const calculateCostVariationPercentage = (budget: number, spent: number): number => {
 ...
 */
export const calculateBudgetUtilization = (budget: number, spent: number): number => {
  if (budget === 0) return 0;
 ...
 */
export const getRiskPriority = (riskScore: number): 'Low' | 'Medium' | 'High' | 'Critical' => {
 ...
 */
export const getBudgetHealthStatus = (utilization: number): 'healthy' | 'warning' | 'critical' => {
 ...
 * - <= 50% = Red (Delayed)
 */
export const getLandAcquisitionStatusColor = (
  percentage: number
): 'green' | 'yellow' | 'red' => {
 ...
 */
export const calculateCostForecast = (
  actualCosts: number[],
 ...
 */
export const calculateForecastVariance = (forecast: number, actual: number): number => {
 ...
 */
export const aggregateBudgetBySource = (budgets: Budget[]): Record<string, number> => {
 ...
 */
export const aggregateBudgetByUtilization = (budgets: Budget[]): Record<string, number> => {
  return budgets.reduce((acc, budget) => {
 ...
 */
export const calculateTotalProjectBudget = (budgets: Budget[]): number => {
 ...
 */
export const calculateTotalProjectSpent = (budgets: Budget[]): number => {
 ...
 */
export const getNextWorkflowStep = (currentStep: number, totalSteps: number): number | null => {
 ...
 */
export const isWorkflowStepRequired = (stepRequired: boolean): boolean => {
  return stepRequired;
};
 ...
 */
export const canApproveDocuments = (userRole: string): boolean => {
  return userRole === 'SPV_Official' || userRole === 'PMNC_Team';
};
 ...
 */
export const filterDocumentsByProject = (
  documents: any[],
  projectId: string
): any[] => {
 ...
 */
export const validateBudgetAllocation = (amount: number): boolean => {
  return amount > 0;
};
 ...
 */
export const validateReimbursementAmount = (amount: number): boolean => {

...
export const LeafletMap = ({ projects, gisFeatures }: LeafletMapProps) => {
  ...
  const center: [number, number] = [17.6816, 77.6077];
  const zoom = 13;
      ...
      <MapContainer center={center} zoom={zoom} className="h-[600px] w-full rounded-lg z-0">
        ...
        {gisFeatures.map((feature) => {
          if (feature.geometry.type === 'Point') {
            const [lng, lat] = feature.geometry.coordinates[0] as number[];
            return (
            ...
            const coords = (feature.geometry.coordinates[0] as number[][]).map(([lng, lat]) => [lat, lng] as [number, number]);
            return (
              <Polygon
                key={feature.id}
                positions={coords}
            ...
            const coords = (feature.geometry.coordinates as number[][]).map(([lng, lat]) => [lat, lng] as [number, number]);
            return (
              <Polyline
                key={feature.id}
                positions={coords}

    ...
    return createPortal(
        <div className="fixed inset-0 z-[9999] bg-slate-900/60 dark:bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
            ...
            >
                ...
                {/* Tabs */}
                <div className="flex border-b border-slate-200 dark:border-neutral-700 px-4">
                    ...
                    <AnimatePresence mode="wait">
                            ...
                            >
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="p-3 bg-slate-50 dark:bg-neutral-800 rounded-lg">
                                        <p className="text-xs text-slate-500 dark:text-neutral-400 mb-1">Document Number</p>
                                        <p className="font-medium text-slate-800 dark:text-white">{document.document_number || '-'}</p>
                                    </div>
                                    <div className="p-3 bg-slate-50 dark:bg-neutral-800 rounded-lg">
                                        <p className="text-xs text-slate-500 dark:text-neutral-400 mb-1">Folder</p>
                                        <p className="font-medium text-slate-800 dark:text-white">{document.folder_path || 'Root'}</p>
                                    </div>
                                    <div className="p-3 bg-slate-50 dark:bg-neutral-800 rounded-lg">
                                        <p className="text-xs text-slate-500 dark:text-neutral-400 mb-1">Uploaded By</p>
                                        <p className="font-medium text-slate-800 dark:text-white">{document.uploaded_by_name}</p>
                                    </div>
                                    <div className="p-3 bg-slate-50 dark:bg-neutral-800 rounded-lg">
                                        <p className="text-xs text-slate-500 dark:text-neutral-400 mb-1">Uploaded</p>
                                ...
                                {document.description && (
                                    <div className="p-3 bg-slate-50 dark:bg-neutral-800 rounded-lg">
                                        <p className="text-xs text-slate-500 dark:text-neutral-400 mb-1">Description</p>
                                        <p className="text-sm text-slate-700 dark:text-slate-300">{document.description}</p>
                                    </div>
                                )}
                                ...
                                {document.current_version && (
                                    <div className="p-3 bg-blue-50 dark:bg-blue-900/10 rounded-lg">
                                        <p className="text-xs text-blue-600 dark:text-blue-400 mb-1">Current Version</p>
                                        <div className="flex items-center justify-between">
                                            <div>
                                                <p className="font-medium text-slate-800 dark:text-slate-200">{document.current_version.file_name}</p>
                                ...
                                {document.versions?.map((version, index) => (
                                    <div
                                        ...
                                        className={`p-3 rounded-lg border ${version.id === document.current_version?.id
                                            ? 'border-primary-200 dark:border-primary-900/50 bg-primary-50 dark:bg-primary-900/10'
                                    ...
                                    >
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="font-medium text-sm text-slate-800 dark:text-slate-200">{version.file_name}</p>
                                                    <p className="text-xs text-slate-500 dark:text-neutral-400">
                                                        By {version.uploaded_by_name} • {formatDistanceToNow(new Date(version.created_at), { addSuffix: true })}
                                                    </p>
                                                </div>
                                            </div>
                                            <Button variant="ghost" size="sm" onClick={handleDownload} className="dark:text-neutral-300 dark:hover:bg-neutral-800">
                                                <Download size={14} />
                                            </Button>
                                        </div>
                                        {version.change_notes && (
                                            <p className="text-xs text-slate-600 dark:text-neutral-400 mt-2 italic">
                                                "{version.change_notes}"
                                            </p>
                                        )}
                                    </div>
                        ...
                        {activeTab === 'workflow' && (
                            ...
                            >
                                        ...
                                        {document.workflow.steps?.map((step, index) => {
                                            ...
                                            return (
                                                <div key={step.id} className="flex items-start gap-3">
                                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isComplete ? 'bg-green-100 dark:bg-green-900/20' : 'bg-slate-100 dark:bg-neutral-800'
                                                        }`}>
                                                        {isComplete ? (
                                                            <CheckCircle size={16} className="text-green-600 dark:text-green-400" />
                                                        ) : (
                                                            <Clock size={16} className="text-slate-400 dark:text-neutral-500" />
                                                        )}
                                                    </div>
                                                    <div className="flex-1">
                                                        ...
                                                        {isComplete ? (
                                                            ...
                                                            <p className="text-xs text-slate-400 dark:text-neutral-500">Pending</p>
                                                        )}
                                                        {step.comments && (
                                                            <p className="text-xs text-slate-600 dark:text-neutral-400 mt-1 italic">"{step.comments}"</p>
                                                        )}
                                                    </div>
                                                </div>
                                            );
                                ...
                                {/* Workflow Actions */}
                                <div className="border-t border-slate-200 dark:border-neutral-700 pt-4 space-y-3">
                                    {/* Submit for Review */}
                                    {canSubmit && (
                                    ...
                                    {canValidate && (
                                        <>
                                            <textarea
                                                ...
                                                className="w-full px-3 py-2 border border-slate-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 text-slate-800 dark:text-white rounded-lg text-sm resize-none"
                                                rows={2}
                                            />
                                            <div className="flex gap-2">
